...
class mafa2kitti():
    ...
    def extract_labels(self, i, train_flag, _count_mask, _count_no_mask):
        ...
        if train_flag:
            ...
            if bboxes:
                img = os.path.join(self.mafa_base_dir , train_image_name)
                #import pdb;pdb.set_trace()
                #print(train_image_name, bboxes)
                #self.save_images(img, train_image_name, bboxes)

                if not self.check_image_dims(image_name=train_image_name):
                    self.make_labels(image_name=train_image_name, category_names=categories, bboxes=bboxes)
    ...
    def save_images(self, image_location, image_name, bboxes, debug=False):
        ...
        image.save(os.path.join(self.kitti_images, image_name), 'JPEG')
