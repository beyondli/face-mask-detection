                ...
                if any(s in line for s in strings) and _count_no_mask < self.count_no_mask:
                    ...
                    for j in range(1, num_faces + 1):
                        ...
                        if bbox:
                            ...
                            #print(image_file_location, image_name, "NOT added to bboxes")
                            pass
                        #bboxes.append(bbox)
                        #category_names.append(category_name)
                    if bboxes:
                       #self.save_images(image_file_location, image_name, bboxes)
                       self.make_labels(image_name=image_name, category_names=category_names, bboxes=bboxes)
                    #_count_no_mask+=1
        return _count_mask, _count_no_mask

    def save_images(self, image_location, image_name_with_path, bboxes, debug=False):

        if debug:
            import pdb;pdb.set_trace()
            print(image_name_with_path, bboxes)

        file_name = os.path.splitext(os.path.split(image_location)[1])[0]
        image = Image.open(os.path.join(self.fddb_base_dir, image_name_with_path)).convert("RGB")
        ...
        image.save(os.path.join(self.kitti_images, file_name + ".jpg"), 'JPEG')
